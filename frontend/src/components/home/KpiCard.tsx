 ...
 * phone. In a card narrower than 11.5rem the tile moves above the text, so the label keeps
 * the full width instead of being cut to "Total emplo…".
 */
export default function KpiCard({
  ...
  const body = (
    <div className="@container h-full">
      <div className="relative flex h-full flex-col gap-3 @min-[11.5rem]:flex-row @min-[11.5rem]:items-start @min-[11.5rem]:gap-4">
        <span
          className={cn("grid size-10 shrink-0 place-items-center rounded-xl @min-[11.5rem]:size-[3.375rem] @min-[11.5rem]:rounded-[0.875rem]", tintStyles[tint])}
          aria-hidden="true"
        >
          <Icon className="size-[1.125rem] @min-[11.5rem]:size-[1.375rem]" />
        </span>
        <div className="relative min-w-0 flex-1">
          <p className="text-[0.8125rem] leading-5 text-fg-muted @min-[11.5rem]:truncate">{label}</p>
          <p className="mt-1 text-[1.625rem] font-semibold leading-8 tracking-tight text-fg tabular-nums @min-[11.5rem]:text-[1.75rem]">
            {isLoading ? <span className="text-fg-subtle">&mdash;</span> : value}
          </p>
          {detail && !isLoading && <div className="mt-1.5 text-[0.8125rem] leading-5 text-fg-muted @min-[11.5rem]:truncate">{detail}</div>}
        </div>
        {aside && !isLoading && (
          <div className="pointer-events-none absolute -bottom-1 right-0 hidden @min-[15rem]:block">{aside}</div>
        )}
        {to && showArrow && (
          <ArrowRight size={18} className="hidden self-center text-fg-subtle @min-[11.5rem]:block" aria-hidden="true" />
        )}
      </div>
    </div>
  );
